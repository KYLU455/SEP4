...
DROP TABLE TAFLIGHTS1;
DROP TABLE TAFLIGHTS2;

--Table with the flights from the club vejle and add a new column with the club name
CREATE TABLE TAFLIGHTS1 AS select * from taflightsvejle ;
alter table taflights1  add (
 club_name varchar(50) );
 
 update TAFLIGHTS1 set club_name='Vejle';
 
 --Table with the flights from the club ssg70 and add a new column with the club name
CREATE TABLE TAFLIGHTS2 AS select * from taflightssg70 ;
alter table TAFLIGHTS2  add (
 club_name varchar(50) );
 
 update TAFLIGHTS2 set club_name='SG70';
 
--the same procedure can be done for more clubs with the same columns name
drop table TAFLIGHTS;

--Table with all the flights with their respective club name
CREATE TABLE TAFLIGHTS as select * from TAFLIGHTS1 union select * from TAFLIGHTS2;


drop table taFlightsYesterday;
create table taFlightsYesterday 
as select * from TAFLIGHTS where 1=0;
...
drop table taFlightsYesterday;
create table taFlightsYesterday 
as select * from TAFLIGHTS ;

--Delete the flights that the ETL used before
DROP TABLE TAFLIGHTS1;
DROP TABLE TAFLIGHTS2;

--Create the tables with the all the information from today:
--Table with the flights from the club vejle and add a new column with the club name
CREATE TABLE TAFLIGHTS1 AS select * from taflightsvejle ;
alter table taflights1  add (
 club_name varchar(50) );
