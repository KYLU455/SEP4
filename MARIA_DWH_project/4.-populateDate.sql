...
--select max(landingtime) from newFlights;, because there are some swapped times
--select max(launchtime) from newFlights;
--select min(launchtime) from newFlights;

declare
  currentDate date := to_date('2014-01-01','YYYY-MM-DD');
  lastDate  date := to_date('2018-01-01','YYYY-MM-DD');
  zyear      int := 0;
  zmonth     int := 0;
  zday       int := 0;
  zhour      int := 0;
